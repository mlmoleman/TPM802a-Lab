...
def parallel_plots_many_policies(
    ...
    for i, solution_index in enumerate(solution_indices):
        norm_df.loc[solution_index, "Name"] = solution_names[i]
        #print(norm_df.loc[solution_index, "Name"])
        concat_rows.append(norm_df.loc[solution_index, :].copy())
        norm_df = pd.concat([norm_df, pd.DataFrame(concat_rows)])

    # norm_df["Name"] = "All Solutions"
    # concat_rows = []
    # for i, solution_indeces in enumerate(solution_indices):
    #     for solution_index in solution_indeces: 
    #         norm_df.loc[solution_index, "Name"] = solution_names[i]
    #         concat_rows.append(norm_df.loc[solution_index, :].copy())
    #         norm_df = pd.concat([norm_df, pd.DataFrame(concat_rows)])
